...
def viewportUI():
	...
	for text in lsText:
		...
		for letter in text:
			cmds.planarSrf('Char_{0}_1'.format(letter), name = "Char_{0}_1_tetris".format(letter) ,ch= 0,tol= 0.01, o= 1 ,po= 0)[0]
	...
	#hud score
	cmds.headsUpDisplay('tetrisScoreHUD',section= 0, block= 11, blockSize = "small", label ="Score :", labelFontSize = "large", command = "scoreTetrisP()", dfs= "large", atr=1)

	#hud level
	cmds.headsUpDisplay('tetrisLevelHUD',section= 0, block= 12, blockSize = "small", label ="Level :", labelFontSize = "large", command = "levelTetrisP()", dfs= "large", atr=1)

	#hud lines
	cmds.headsUpDisplay('tetrisLinesHUD',section= 0, block= 13, blockSize = "small", label ="Lines :", labelFontSize = "large", command = "linesTetrisP()", dfs= "large", atr=1)
...
def hotkeySet():
	...
	lsValidHotkey= ['a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z','Up','Down','Right','Left','Home','End','Page_Up','Page_Down','Insert','Return','Space']
...
def tetriminoMaker(idT, partPos):
	...
	while i < 4:
		pos = partPos[i]
		namePart = cmds.polyCube(w=1, h=1,d=1, sx=1, sy=1, sz=1, ax =[0,1,0], ch=0, cuv= 0, n = "tetrimino{1}{0}part1".format(idT, partPos[4]))
...
def moveTetris(moveXY, activeTetrimino, activePart, condition, channel):
	...
	for part in activePart:
		...
		if (posPart[channel] == condition[0] or matrixPos[int(posPart[1])+condition[1]][int(posPart[0]+condition[2])] == 1): #teste si la place dans la directoin du deplacement est occupe
			limit = 1
			break
	if (limit== 0 ):
...
def rotateTetris(activeTetrimino, activePart):
	...
	for i in range(3):
		newPos=[]
		for pos in posPart:
			resultRot = rotate([originPos[0]+offset[0],originPos[1]+offset[1]], [pos[0]+offset[0],pos[1]+offset[1]], math.radians(-90))
...
def runGame():
			...
			while i <20:
				...
				for f in range (10): #actualisation
					...
					if (action == 1): #leftmove
						...
						activeTetrimino= holdTetris(activeTetrimino,activePart , lsTetriminoHold[lsTetriminoName.index(activeTetrimino[0:10])])
						if (activeTetrimino ==""):
							activePart=[]
							i=0
							break
				...
				for part in activePart:
					posPart = cmds.xform(part, ws=1, q=1, t=1)
					if (posPart[1] ==0 or matrixPos[int(posPart[1]-1)][int(posPart[0]+4)] == 1): # teste si le sol ou un tetrimino est deja en dessous
						limit = 1 
						break
				...
				if (limit == 1):
					for part in activePart:
						posPart = cmds.xform(part, ws=1, q=1, t=1)
						matrixPos[int(posPart[1])][int(posPart[0]+4)] = 1 #actualise les matrices avec les nouvelles cases occupees
	...
	cmds.refresh()
	cmds.confirmDialog(title = "Score", button="Ok", message= "Game Over\n\n Final Score: {0}\n Lines: {1}\n Final Level: {2}".format(scoreTetris, totalLines, levelTetris))
...
cmds.window('mainTetrisUI', edit= 1,h= 300, w=150, maximizeButton=0, minimizeButton=0, sizeable=0, title="Tetris for Maya", cc= 'cleanGame()\ncmds.windowPref("mainTetrisUI", remove=True )') 
